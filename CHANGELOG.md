...
* Fixed black border appearing when switching pages with collapsed group tasks. The axis date range is now computed from all tasks with dates instead of only visible grouped tasks, so the axis always renders even when all groups are collapsed.
